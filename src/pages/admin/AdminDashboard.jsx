...
/* ─────────────────────────────────────────────
   GLOBAL STYLES (keyframes + mobile resets)
   Injected into <head> once so portal elements
   (Toast, Modal) can use the animations.
───────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────
   TOAST — portal-rendered, full-width on mobile
───────────────────────────────────────────── */
function Toast({ toasts, removeToast }) {
  return ReactDOM.createPortal(
    <div style={{
      ...
      padding: '10px 12px 0', pointerEvents: 'none',
    }}>
      {toasts.map(t => (
        <div key={t.id} style={{
          pointerEvents: 'all',
          display: 'flex', alignItems: 'flex-start', gap: 12,
          background: t.type === 'error' ? '#1a0505' : '#0f1117',
...
/* ─────────────────────────────────────────────
   PASSWORD RESET MODAL — portal-rendered
───────────────────────────────────────────── */
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: '#fef3c7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>🔑</div>
          <div>
        ...
        {[
          ...
          <div key={label} style={{ marginBottom: 14 }}>
            ...
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              ...
              />
              <button type="button" onClick={toggle} style={{ position: 'absolute', right: 12, background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, color: '#6b7280', padding: 4 }}>
                {show ? '👁️' : '👁️‍🗨️'}
              </button>
            </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '13px', border: '1.5px solid #e5e7eb', borderRadius: 10, background: '#fff', color: '#374151', fontSize: 14.5, fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}
            onMouseEnter={e => e.currentTarget.style.background = '#f9fafb'}
            onMouseLeave={e => e.currentTarget.style.background = '#fff'}
          >Cancel</button>
          <button onClick={onSubmit} style={{ flex: 1, padding: '13px', border: 'none', borderRadius: 10, background: '#0f1117', color: '#fff', fontSize: 14.5, fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}
...
function AdminDashboard({ drivers: propDrivers, trips, fetchTrips }) {
  ...
  const fetchHistoricalData = async () => {
    ...
    try {
      ...
      addToast('Failed to load historical data.', 'error', 'Load Error');
    } finally {
  ...
  const handleResetPassword = async () => {
    if (!newPassword)              { addToast('Please enter a new password',          'error', 'Password Required'); return; }
    if (newPassword.length < 6)    { addToast('Password must be at least 6 characters', 'error', 'Invalid Password');  return; }
    if (newPassword !== confirmPassword) { addToast('Passwords do not match', 'error', 'Password Mismatch'); return; }
    try {
  ...
  const analyticsData = useMemo(() => {
    ...
    if (analyticsPeriod === 'day') {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        tripCounts.push(wt.length);
        kmTotals.push(wt.reduce((s, t) => s + (parseFloat(t.odometer) || 0), 0));
        amountTotals.push(wt.reduce((s, t) => s + getAmount(t), 0));
      }
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        tripCounts.push(mt.length);
        kmTotals.push(mt.reduce((s, t) => s + (parseFloat(t.odometer) || 0), 0));
        amountTotals.push(mt.reduce((s, t) => s + getAmount(t), 0));
      }
    } else {
      ...
      for (let y = startYear; y <= refYear; y++) {
        ...
        tripCounts.push(yt.length);
        kmTotals.push(yt.reduce((s, t) => s + (parseFloat(t.odometer) || 0), 0));
        amountTotals.push(yt.reduce((s, t) => s + getAmount(t), 0));
      }
    }
  ...
  const topDrivers = useMemo(() => {
    const map = {};
    analyticsTrips.forEach(t => { if (!t.driver_name) return; if (!map[t.driver_name]) map[t.driver_name] = { trips: 0, km: 0 }; map[t.driver_name].trips++; map[t.driver_name].km += parseFloat(t.odometer) || 0; });
  ...
  const topDealersByAverage = useMemo(() => {
    const map = {};
    analyticsTrips.forEach(t => { if (!t.dealer) return; if (!map[t.dealer]) map[t.dealer] = { total: 0, count: 0 }; map[t.dealer].total += getAmount(t); map[t.dealer].count++; });
    return Object.entries(map).map(([name, d]) => ({ name, average: d.count > 0 ? d.total / d.count : 0, ...d })).filter(d => d.count >= 2).sort((a, b) => b.average - a.average);
  }, [analyticsTrips]);
    ...
    <>
      <style>{`
        ...
        *{-webkit-tap-highlight-color:transparent;box-sizing:border-box}
        ...
        .ad-db-btn{display:flex;align-items:center;gap:7px;padding:8px 16px;border:1.5px solid #e5e7eb;border-radius:9px;background:#fff;font-size:13px;font-weight:600;color:#6b7280;cursor:pointer;transition:all .2s;font-family:'Syne',sans-serif;-webkit-appearance:none}
        ...
        .ad-db-info{margin-left:auto;font-size:12px;color:#9ca3af;display:flex;align-items:center;gap:7px}
        ...
        /* ── Tabs ── */
        .ad-tabs{display:flex;gap:3px;background:#f3f4f6;border-radius:10px;padding:4px;margin-bottom:16px;overflow-x:auto;-webkit-overflow-scrolling:touch}
        .ad-tabs::-webkit-scrollbar{display:none}
        .ad-tab{display:flex;align-items:center;gap:7px;padding:9px 16px;border-radius:7px;border:none;background:transparent;color:#6b7280;font-size:13px;font-weight:500;font-family:'DM Sans',sans-serif;cursor:pointer;transition:all .18s;white-space:nowrap;-webkit-appearance:none;flex-shrink:0}
        .ad-tab:hover{color:#374151}
        .ad-tab.active{background:#0f1117;color:#fff;box-shadow:0 1px 4px rgba(0,0,0,.15)}
        .ad-tab-count{background:rgba(245,158,11,.15);color:#f59e0b;font-size:11px;font-weight:700;padding:2px 7px;border-radius:20px;font-family:'Syne',sans-serif}
        ...
        .ad-search{padding:9px 13px 9px 32px;border:1.5px solid #e5e7eb;border-radius:8px;font-size:14px;font-family:'DM Sans',sans-serif;color:#111827;background:#f9fafb;outline:none;width:100%;-webkit-appearance:none;transition:all .18s}
        ...
        .ad-export-btn{display:flex;align-items:center;gap:6px;background:#0f1117;border:none;border-radius:8px;padding:9px 14px;font-size:13px;font-family:'DM Sans',sans-serif;font-weight:500;color:#fff;cursor:pointer;white-space:nowrap;-webkit-appearance:none}
        .ad-export-btn:active{background:#1f2937}
        .ad-refresh-btn{display:flex;align-items:center;gap:6px;background:#f3f4f6;border:none;border-radius:8px;padding:9px 14px;font-size:13px;font-family:'DM Sans',sans-serif;font-weight:500;color:#4b5563;cursor:pointer;white-space:nowrap;-webkit-appearance:none}
        .ad-refresh-btn:active{background:#e5e7eb}
        .ad-clear-btn{display:flex;align-items:center;gap:5px;background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:9px 12px;font-size:13px;font-family:'DM Sans',sans-serif;font-weight:500;color:#ef4444;cursor:pointer;white-space:nowrap;-webkit-appearance:none}
        .ad-clear-btn:active{background:#fee2e2}
        ...
        .ad-table td{padding:13px 16px;color:#374151;border-bottom:1px solid #f3f4f6;white-space:nowrap;font-size:13.5px}
        ...
        .ad-map-link{color:#3b82f6;text-decoration:none;display:inline-flex;align-items:center;gap:4px;padding:3px 9px;background:#eff6ff;border-radius:6px;font-size:12px;font-weight:500;transition:all .18s}
        ...
        /* ── Analytics ── */
        .an-header{padding:20px 24px;border-bottom:1px solid #f3f4f6;display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:12px}
        .an-title{font-family:'Syne',sans-serif;font-size:16px;font-weight:700;color:#0f1117;letter-spacing:-.2px}
        .an-sub{font-size:12.5px;color:#9ca3af;margin-top:4px}
        ...
        .an-period-btn{padding:7px 13px;border-radius:7px;border:none;background:transparent;font-size:12.5px;font-family:'DM Sans',sans-serif;font-weight:500;color:#6b7280;cursor:pointer;transition:all .15s;-webkit-appearance:none;white-space:nowrap}
        .an-period-btn.active{background:#0f1117;color:#fff;box-shadow:0 1px 3px rgba(0,0,0,.15)}
        .an-period-btn:not(.active):hover{color:#374151;background:#e9eaec}
        .an-body{padding:22px;display:flex;flex-direction:column;gap:20px}
        .an-kpi-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}
        .an-kpi{border-radius:14px;padding:18px 20px;border:1px solid #e5e7eb;background:#f8fafc}
        .an-kpi.accent{background:#0f1117;border-color:#0f1117}
        .an-kpi-label{font-size:11.5px;color:#9ca3af;margin-bottom:9px;font-weight:500}
        .an-kpi.accent .an-kpi-label{color:#6b7280}
        .an-kpi-val{font-family:'Syne',sans-serif;font-size:26px;font-weight:800;color:#0f1117;letter-spacing:-1px;line-height:1}
        .an-kpi.accent .an-kpi-val{color:#f59e0b}
        .an-kpi-sub{font-size:11.5px;color:#10b981;margin-top:6px}
        .an-kpi.accent .an-kpi-sub{color:#6b7280}
        .an-charts{display:grid;grid-template-columns:1fr 1fr;gap:16px}
        .an-chart-card{background:#f8fafc;border:1px solid #e5e7eb;border-radius:14px;padding:20px 22px}
        .an-chart-title{font-family:'Syne',sans-serif;font-size:14.5px;font-weight:700;color:#0f1117;margin-bottom:3px;letter-spacing:-.2px}
        .an-chart-sub{font-size:11.5px;color:#9ca3af;margin-bottom:18px}
        .an-bar-chart{display:flex;align-items:flex-end;gap:4px;height:160px;overflow-x:auto;padding-bottom:2px;-webkit-overflow-scrolling:touch}
        .an-bar-chart::-webkit-scrollbar{height:4px}
        .an-bar-chart::-webkit-scrollbar-thumb{background:#e5e7eb;border-radius:2px}
        .an-bar-col{display:flex;flex-direction:column;align-items:center;gap:4px;flex:1;min-width:32px}
        .an-bar-wrap{width:100%;display:flex;flex-direction:column;justify-content:flex-end;height:110px}
        .an-bar{width:80%;margin:0 auto;border-radius:4px 4px 0 0;transition:height .4s ease;min-height:3px;cursor:pointer}
        .an-bar:hover{filter:brightness(1.12)}
        .an-bar-label{font-size:10px;color:#9ca3af;white-space:nowrap}
        .an-bar-val{font-size:10.5px;font-weight:700;color:#6b7280;font-family:'Syne',sans-serif;line-height:1}
        .an-leaderboard{display:flex;flex-direction:column;gap:11px}
        .an-lb-row{display:flex;align-items:center;gap:9px}
        .an-lb-rank{font-family:'Syne',sans-serif;font-size:12.5px;font-weight:800;color:#d1d5db;width:16px;flex-shrink:0;text-align:center}
        .an-lb-rank.top{color:#f59e0b}
        .an-lb-avatar{width:28px;height:28px;border-radius:7px;display:flex;align-items:center;justify-content:center;font-family:'Syne',sans-serif;font-weight:700;font-size:11px;flex-shrink:0}
        ...
        .an-lb-name{font-size:12.5px;font-weight:500;color:#111827;width:90px;flex-shrink:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        .an-lb-bar-wrap{flex:1;height:6px;background:#e5e7eb;border-radius:4px;overflow:hidden}
        .an-lb-bar{height:100%;border-radius:4px;transition:width .5s ease}
        ...
        .an-lb-val{font-size:12.5px;font-weight:700;color:#374151;width:64px;text-align:right;flex-shrink:0;font-family:'Syne',sans-serif}
        .an-empty{padding:40px 20px;text-align:center;color:#9ca3af;font-size:13.5px}
        ...
        @media(max-width:640px){
          ...
          /* Database switcher — wrap nicely */
          .ad-db-switcher{padding:12px 14px;gap:8px;border-radius:12px;margin-bottom:12px}
          .ad-db-label{font-size:11px}
          .ad-db-btn{padding:8px 14px;font-size:12.5px}
          .ad-db-info{margin-left:0;width:100%}

          /* Stats — 2x2 grid */
          .ad-stats{grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px}
          .ad-stat-card{padding:12px 14px;gap:10px;border-radius:12px}
          .ad-stat-icon{width:36px;height:36px;border-radius:9px}
          .ad-stat-icon svg{width:17px;height:17px}
          .ad-stat-value{font-size:20px}
          .ad-stat-label{font-size:10px}

          /* Tabs — horizontal scroll */
          .ad-tabs{margin-bottom:14px;border-radius:9px}
          .ad-tab{padding:8px 14px;font-size:12.5px}

          /* Card header — stack */
          .ad-card-header{padding:14px 16px;flex-direction:column;gap:10px}
          .ad-actions{width:100%;flex-direction:column;gap:8px}
          .ad-search-wrap{width:100%;min-width:0}
          .ad-search{width:100%;font-size:16px}
          .ad-select,.ad-date-input{width:100%;font-size:16px;padding:11px 12px}
          .ad-select{padding-right:30px}
          .ad-export-btn,.ad-refresh-btn,.ad-clear-btn{width:100%;justify-content:center;padding:12px;font-size:13.5px}

          /* Filter bar */
          .ad-filter-bar{padding:9px 16px}
          .ad-results-count{padding:8px 16px}

          /* Mobile card-based table for DRIVERS */
          .ad-table-drivers thead{display:none}
          .ad-table-drivers,.ad-table-drivers tbody,.ad-table-drivers tr,.ad-table-drivers td{display:block;width:100%}
          .ad-table-drivers tbody tr{
            border:1px solid #e5e7eb;border-radius:12px;
            margin:0 16px 10px;width:calc(100% - 32px);
            padding:14px;background:#fff;
            box-shadow:0 1px 4px rgba(0,0,0,.05);
          }
          .ad-table-drivers tbody tr:last-child{margin-bottom:16px}
          .ad-table-drivers td{padding:5px 0;border-bottom:1px solid #f7f7f7;font-size:13px;display:flex;align-items:center;gap:8px;white-space:normal}
          .ad-table-drivers td:first-child{padding-bottom:10px;border-bottom:1.5px solid #f3f4f6;margin-bottom:4px}
          .ad-table-drivers td:last-child{border-bottom:none;padding-top:10px;justify-content:flex-start}
          .ad-table-drivers td::before{content:attr(data-label);font-size:10px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.6px;flex-shrink:0;width:60px}
          .ad-table-drivers td:first-child::before,.ad-table-drivers td:last-child::before{display:none}
          .ad-driver-cell{flex:1}
          .ad-driver-avatar{width:38px;height:38px;font-size:14px;border-radius:10px}
          .ad-driver-name{font-size:14px}

          /* Mobile card-based table for TRIPS */
          .ad-table-trips thead{display:none}
          .ad-table-trips,.ad-table-trips tbody,.ad-table-trips tr,.ad-table-trips td{display:block;width:100%}
          .ad-table-trips tbody tr{
            border:1px solid #e5e7eb;border-radius:12px;
            margin:0 16px 10px;width:calc(100% - 32px);
            padding:14px;background:#fff;
            box-shadow:0 1px 4px rgba(0,0,0,.05);
          }
          .ad-table-trips tbody tr:last-child{margin-bottom:16px}
          .ad-table-trips td{padding:5px 0;border-bottom:1px solid #f7f7f7;font-size:13px;display:flex;align-items:flex-start;gap:8px;white-space:normal}
          .ad-table-trips td:first-child{padding-bottom:8px;border-bottom:1.5px solid #f3f4f6;margin-bottom:4px}
          .ad-table-trips td:last-child{border-bottom:none;padding-top:8px}
          .ad-table-trips td::before{content:attr(data-label);font-size:10px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.6px;flex-shrink:0;width:64px;padding-top:1px}
          .ad-table-trips td:first-child::before{display:none}

          /* Analytics */
          .an-header{padding:14px 16px}
          .an-filters{width:100%}
          .an-period-group{width:100%;justify-content:stretch}
          .an-period-btn{flex:1;text-align:center;padding:8px 6px;font-size:12px}
          .an-filters .ad-select{width:100%;font-size:16px}
          .an-filters .ad-refresh-btn{width:100%;justify-content:center;padding:11px}
          .an-body{padding:14px;gap:14px}
          .an-kpi-grid{grid-template-columns:1fr 1fr;gap:10px}
          .an-kpi{padding:14px 16px}
          .an-kpi-val{font-size:22px}
          .an-charts{grid-template-columns:1fr;gap:12px}
          .an-chart-card{padding:16px}
          .an-lb-name{width:70px}
          .an-lb-val{width:55px;font-size:12px}
        }
        ...
        @media(max-width:380px){
          ...
          .ad-stat-card{padding:10px 12px;gap:8px}
          .ad-stat-value{font-size:18px}
          .ad-stat-icon{width:32px;height:32px}
          .an-kpi-val{font-size:19px}
        }
      `}</style>
      ...
      <div className="ad-root">
        ...
        <div className="ad-db-switcher">
          ...
          {loadingHistorical && activeDatabase === 'historical' && (
            <span className="ad-db-info">
              <div style={{ width: 13, height: 13, borderRadius: '50%', border: '2px solid #e5e7eb', borderTopColor: '#f59e0b', animation: 'spin 0.75s linear infinite', flexShrink: 0 }} />
              Loading 2025 data…
            </span>
          )}
        </div>
        ...
        <div className="ad-tabs">
          <button className={`ad-tab${view === 'drivers' ? ' active' : ''}`} onClick={() => setView('drivers')}>
            <UsersIcon size={14} /> Drivers <span className="ad-tab-count">{currentDrivers.length}</span>
          </button>
          <button className={`ad-tab${view === 'trips' ? ' active' : ''}`} onClick={() => setView('trips')}>
            <TruckIcon size={14} /> All Trips <span className="ad-tab-count">{currentTrips.length}</span>
          </button>
              ...
              : filteredDrivers.length === 0
                ...
                : (
                  <div className="ad-table-wrap">
                    <table className="ad-table ad-table-drivers">
                      <thead>
                        <tr><th>Driver</th><th>Email</th><th>Phone</th><th>License No.</th><th>Actions</th></tr>
                      </thead>
                      <tbody>
                          ...
                          <tr key={driver.id}>
                            ...
                            <td>
                              <button
                                ...
                                style={{ background: '#fef3c7', border: 'none', borderRadius: 7, padding: '8px 14px', fontSize: 12.5, fontWeight: 600, color: '#92400e', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5, WebkitAppearance: 'none' }}
          ...
          <div className="ad-card">
            ...
            {loadingHistorical && activeDatabase === 'historical'
              ...
              : filteredTrips.length === 0
                ...
                : (
                  <div className="ad-table-wrap">
                    <table className="ad-table ad-table-trips">
                      <thead>
                        <tr><th>Date</th><th>Driver</th><th>Helper</th><th>Dealer</th><th>Time In</th><th>Time Out</th><th>Odometer</th><th>Invoices</th><th>Checks</th><th>Location</th></tr>
                      </thead>
                      <tbody>
                        {filteredTrips.map(trip => (
                          <tr key={trip.id}>
                            <td><span style={{ fontWeight: 600, color: '#0f1117', fontSize: 13.5 }}>{trip.date}</span></td>
                            <td data-label="Driver">
                              <div className="ad-driver-cell">
                                <div className="ad-driver-avatar">{trip.driver_name?.charAt(0).toUpperCase()}</div>
                                <span style={{ fontSize: 13, fontWeight: 500 }}>{trip.driver_name}</span>
                              </div>
                            </td>
                            ...
                            <td data-label="Invoices">
                              {trip.invoices?.length > 0
                                ? <div style={{ flexWrap: 'wrap', display: 'flex', gap: 2 }}>{trip.invoices.map((inv, i) => <span key={i} className="ad-invoice-badge">{inv.invoice_no}: <strong>₱{Number(inv.amount).toLocaleString()}</strong></span>)}</div>
                                : trip.invoice_no
                            ...
                            <td data-label="Checks">
                              {trip.checks?.length > 0
                                ? <div style={{ flexWrap: 'wrap', display: 'flex', gap: 2 }}>{trip.checks.map((chk, i) => <span key={i} className="ad-check-badge">{chk.check_no}: <strong>₱{Number(chk.amount).toLocaleString()}</strong></span>)}</div>
                                : <span style={{ color: '#d1d5db' }}>—</span>}
                            </td>
                            <td data-label="Location">
                              {trip.location_lat && trip.location_lng
                                ? <a href={`https://www.google.com/maps?q=${trip.location_lat},${trip.location_lng}`} target="_blank" rel="noopener noreferrer" className="ad-map-link">📍 View Map</a>
                                : <span style={{ color: '#d1d5db' }}>—</span>}
                            </td>
                          </tr>
                        ))}
            ...
            <div className="an-body">
              ...
              <div className="an-kpi-grid">
                ...
                <div className="an-kpi">
                  ...
                  <div className="an-kpi-sub" style={{ color: '#9ca3af' }}>Per delivery avg</div>
                </div>
                <div className="an-kpi">
              ...
              <div className="an-charts">
                <div className="an-chart-card">
                  ...
                  {topDealersByAmount.length === 0
                    ...
                    : <div className="an-leaderboard">{topDealersByAmount.slice(0, 5).map((d, i) => (
                ...
                <div className="an-chart-card">
                  ...
                  <div className="an-chart-sub">Highest avg invoice per trip (min 2)</div>
                  {topDealersByAverage.length === 0
                    ? <div className="an-empty">No data available</div>
                    : <div className="an-leaderboard">{topDealersByAverage.slice(0, 5).map((d, i) => (
                        <div key={i} className="an-lb-row">
                          <span className={`an-lb-rank${i === 0 ? ' top' : ''}`}>{i+1}</span>
                          <div className="an-lb-avatar indigo">{d.name.charAt(0)}</div>
                          <span className="an-lb-name">{d.name}</span>
              ...
              <div className="an-chart-card">
                ...
                {driverRevenue.length === 0
                  ? <div className="an-empty">No data available</div>
                  : <div className="an-leaderboard" style={{ marginTop: 14 }}>
                      {driverRevenue.slice(0, 5).map((d, i) => (
                        <div key={i} className="an-lb-row">
                          ...
                          <span className="an-lb-val">₱{d.total.toLocaleString()}</span>
                          <span style={{ fontSize: 10.5, color: '#9ca3af', marginLeft: 4, flexShrink: 0 }}>({d.trips})</span>
                        </div>
                      ))}
                    </div>}
              </div>
                      ...
                      return (
                        <div key={i} style={{ marginBottom: 13 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                            <span style={{ fontSize: 12.5, color: '#374151', fontWeight: 500 }}>{dealer.name}</span>
                            <span style={{ fontSize: 12.5, fontWeight: 700, color: '#0f1117', fontFamily: "'Syne', sans-serif" }}>{pct}%</span>
                          </div>
                          <div style={{ height: 7, background: '#e5e7eb', borderRadius: 4 }}>
                            <div style={{ width: `${pct}%`, height: '100%', background: colors[i], borderRadius: 4, transition: 'width .6s ease' }} />
                          </div>
                        </div>
                      );
                    })}
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: i < 2 ? '1px solid #f3f4f6' : 'none' }}>
                          ...
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ fontSize: 13.5, fontWeight: 700, color: '#0f1117', fontFamily: "'Syne', sans-serif" }}>₱{cur.toLocaleString()}</div>
                            {growth !== null && (
                              <div style={{ fontSize: 11, color: growth > 0 ? '#10b981' : growth < 0 ? '#ef4444' : '#9ca3af', fontWeight: 600 }}>
